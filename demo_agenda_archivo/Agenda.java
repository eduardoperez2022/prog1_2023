...
public class Agenda {
    ...
    public void lee_archivo() {
        ...
        archivo_agenda ag = new archivo_agenda();
        ag.leer_archivo();
        for (String linea : ag.getAgenda_array()) {
            String[] c_array = linea.split(",");             
            String elnombre = c_array[0];
            String elnum = c_array[1];
            int elnumero=-1;
            try {
                elnumero = Integer.parseInt(elnum.trim());
            }
            catch (Exception e) {
                System.out.println("Error de numero:" + elnum);
            }
            Anadir(elnombre, elnumero);
        }
        
    }
    
    public void escribe_archivo() {
        
        archivo_agenda ag = new archivo_agenda();
        ag.escribe_archivo(this.lista_contactos);
